...
## Update : Search with expanded parents

Of course. This is a crucial feature for usability. The logic will be primarily handled by the backend, which will receive a search query, find all matching items, trace their parents to the root, and then construct a new tree containing only the necessary items with all parent nodes pre-expanded.

The frontend will be updated to include a search input that triggers this API call.

Here is the complete, final code for all files, with the new search logic integrated.

---

### Part 1: Vue 2 Frontend (Complete Code)

The `TreeDatatable.vue` component itself requires **no changes**, as it is a "controlled" component. It simply renders the `rows` it is given. All the new logic will be in the parent component, `App.vue`.

#### 1.1. `src/components/TreeDatatable.vue` (Unchanged)

This file remains the same as the previous "External Loader" version. I am including it here for completeness.
...
#### 1.2. `src/App.vue` (Complete & Modified Example)

This component now includes the search input and debounced search logic.

```vue
<template>
  <div id="app" class="container mt-4 mb-5">
    <header class="mb-4">
      <h1>Vue 2 Tree Datatable - Search Demo</h1>
      <p>The parent component now controls data fetching, loading, error states, and search functionality.</p>
    </header>
...
// Mocking Axios is the same
const mock = new MockAdapter(axios, { delayResponse: 500 });
const mockData = [ /* ... same data as before ... */ ];
mock.onGet("/api/file-system").reply(config => {
    console.log("Mock API received request with params:", config.params);
    // This mock doesn't simulate search, we rely on the real backend for that logic.
    return [200, mockData];
});
let nextId = 100;
...
export default {
  ...
  watch: {
    // Watch for changes to the search query and debounce the API call
    searchQuery() {
  ...
  methods: {
    ...
    handleRefresh(payload) {
      console.log('Refresh event received from child:', payload);
      // When refreshing, include both the current search query and the expanded state
      this.loadTableData({ expanded: payload.expandedIds });
    },
    refreshTable() {
      this.$refs.treeDatatable.refreshData();
...
The controller now has two distinct logic paths: one for standard paginated requests and one for search requests.

#### 2.1. Controller: `app/Http/Controllers/FileSystemController.php` (Modified)
...
{
    /**
     * Main data entry point. Delegates to search or paginated logic.
     */
    public function getData(Request $request)
    {
        ...
        if ($searchQuery) {
            // If a search query is present, use the search logic
            $tree = $this->getSearchResults($searchQuery);
        } else {
            // Otherwise, use the standard paginated logic
            $page = $request->input('page', 1);
            $perPage = $request->input('perPage', 10);
            $expandedIds = $request->input('expanded');

            $tree = $this->getPaginatedData($page, $perPage, $expandedIds);
        }
    ...
    /**
     * Handles standard paginated requests.
     */
    private function getPaginatedData(int $page, int $perPage, ?array $expandedIds): array
    {
        $rootItemsQuery = FileSystemItem::whereNull('_parentId');
        $paginatedRootItems = (clone $rootItemsQuery)->skip(($page - 1) * $perPage)->take($perPage)->get();
        ...
        return $this->buildTree($allItems, $paginatedRootItems->pluck('id')->toArray(), $expandedIds);
    }

    /**
     * Handles search requests. Finds matches, their ancestors, and builds a filtered tree.
     */
    private function getSearchResults(string $searchQuery): array
    {
        // 1. Find all items that match the search query (case-insensitive)
        $matchedItems = FileSystemItem::where('name', 'like', "%{$searchQuery}%")->get();

        if ($matchedItems->isEmpty()) {
            return [];
        }

        // 2. To find ancestors efficiently, fetch all items and create a map
        $allItems = FileSystemItem::all();
        $itemsById = $allItems->keyBy('id');

        $requiredIds = collect();
        $ancestorIds = collect();

        // 3. For each matched item, traverse up to find all its parents
        foreach ($matchedItems as $item) {
            $requiredIds->put($item->id, true); // Add the matched item itself
            $current = $item;
            while ($current && $current->_parentId) {
                if ($requiredIds->has($current->_parentId)) {
                    break; // Stop if we've already processed this parent branch
                }
                $requiredIds->put($current->_parentId, true);
                $ancestorIds->put($current->_parentId, true); // Mark as an ancestor
                $current = $itemsById->get($current->_parentId);
            }
        }

        // 4. Filter the complete item list to include only required items
        $finalItems = $allItems->whereIn('id', $requiredIds->keys());

        // 5. Identify the root nodes of this new, filtered tree
        $rootIds = $finalItems->filter(function ($item) {
            return !$item->_parentId || !in_array($item->_parentId, $item->pluck('id')->toArray());
        })->pluck('id')->toArray();

        // 6. Build the tree, forcing all ancestors to be expanded
        return $this->buildTree($finalItems, $rootIds, $ancestorIds->keys()->toArray());
    }

    /**
     * Reusable tree builder function.
     */
    private function buildTree(Collection $items, array $rootIds, ?array $expandedIds): array
    {
        $map = [];
...
The API documentation is updated to reflect the new `search` parameter.

### 3.1. Documentation (English)

## `TreeDatatable` Component & API

### Component (`TreeDatatable.vue`)

A reusable Vue 2 component for displaying hierarchical data. It can fetch data internally via a URL or accept data directly via a prop.

**Props:** `url`, `rows`, `columns`, `customActions`.
**Events:** `@action`, `@refresh`.
**Public Methods:** `refreshData()`, `addChildItem()`, `updateItem()`, `deleteItem()`.

*(Full prop, event, and method descriptions are the same as the previous answer)*

### Backend API (`/api/file-system`)

**Method:** `GET`

**Description:**
Provides hierarchical data for the TreeDatatable. Operates in two modes: **Paginated Mode** and **Search Mode**.

**Parameters:**

| Parameter | Type | Optional | Description |
| :--- | :--- | :--- | :--- |
| `page` | Integer | Yes | The page number for root-level items. *Used only in Paginated Mode*. |
| `perPage` | Integer | Yes | The number of root-level items per page. *Used only in Paginated Mode*. |
| `expanded[]` | Array | Yes | An array of item IDs that should be marked as `_expanded: true`. |
| `search` | String | Yes | A search term. **If provided, the API switches to Search Mode.** |

**Modes of Operation:**

1.  **Paginated Mode (Default)**
-   **Trigger:** The `search` parameter is empty or not provided.
-   **Behavior:** Returns a paginated list of root-level items and all their descendants. Pagination applies *only* to the root level.
-   **Example:** `/api/file-system?page=2&perPage=5`

2.  **Search Mode**
-   **Trigger:** The `search` parameter is provided with a value.
-   **Behavior:**
-   Pagination (`page`, `perPage`) is **ignored**.
-   The API searches all items (roots and children) for a match in the `name` field.
-   It returns a filtered tree containing **only** the matched items and their direct parent-to-root ancestors.
-   All ancestor items in the response are automatically marked as `_expanded: true` to ensure the search results are visible.
-   **Example:** `/api/file-system?search=project-beta`